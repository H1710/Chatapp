        ...
        <>
          {messages &&
            messages.map(message => {
              let utcDate: string = new Date(message.updatedAt).toUTCString();
              const time: string[] = utcDate.split(' ');
              let newtime: string[] = time[4].split(':');
              newtime[0] = ((+newtime[0] + 7) % 12).toString();
